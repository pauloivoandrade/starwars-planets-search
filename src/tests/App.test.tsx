...
//   Espera até que o elemento com data-testid="name-filter" seja renderizado no DOM
  ...
  expect(inputNumber).toHaveValue(parseFloat(operator));
...
test('Teste se ao ordenar por population, a ordenacao ascendente e descendente funcionam', async () => {
  render(<App />);
  ...
  const planetName = 'Yavin IV';

//   const names = screen.getByTestId('planet-name');
  const selectColumn = screen.getByTestId('column-sort');
  const selectOrderAsc = screen.getByTestId('column-sort-input-asc');
  const sortButton = screen.getByTestId('column-sort-button');
  expect(selectColumn).toBeInTheDocument();
  expect(selectOrderAsc).toBeInTheDocument();
  expect(sortButton).toBeInTheDocument();

  await userEvent.selectOptions(selectColumn, 'population');
  await userEvent.click(selectOrderAsc)
